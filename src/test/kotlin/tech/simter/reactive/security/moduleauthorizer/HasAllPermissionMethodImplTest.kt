...
class HasAllPermissionMethodImplTest {
  @Test
  fun `empty operations params`() {
    ALLOWER.hasAllPermission()
      .test()
      .expectError(IllegalArgumentException::class.java)
      .verify()

    DENIER.hasAllPermission()
      .test()
      .expectError(IllegalArgumentException::class.java)
      .verify()
  }
  ...
  fun `explicit allow anything`() {
    ALLOWER.hasAllPermission("any1", "any2")
      .test()
      .expectNext(true)
      .verifyComplete()
  }
  ...
  fun `explicit deny anything`() {
    DENIER.hasAllPermission("any1", "any2")
      .test()
      .expectNext(false)
      .verifyComplete()
  }
  ...
  fun `multiple operations`() {
    ...
    listOf(
      ...
      spyModuleAuthorizer.hasAllPermission(pair.first.operation, pair.second.operation)
        .test()
