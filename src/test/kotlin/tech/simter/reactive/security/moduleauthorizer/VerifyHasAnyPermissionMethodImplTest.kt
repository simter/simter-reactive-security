...
class VerifyHasAnyPermissionMethodImplTest {
  @Test
  fun `empty operations params`() {
    ALLOWER.verifyHasAnyPermission()
      .test()
      .verifyError(IllegalArgumentException::class.java)

    DENIER.verifyHasAnyPermission()
      .test()
      .verifyError(IllegalArgumentException::class.java)
  }
  ...
  fun `explicit allow anything`() {
    ALLOWER.verifyHasAnyPermission("any1", "any2")
      .test()
      .verifyComplete()
  }
  ...
  fun `explicit deny anything`() {
    DENIER.verifyHasAnyPermission("any1", "any2")
      .test()
  ...
  fun `multiple operations`() {
    ...
    listOf(
      ...
      val test = spyModuleAuthorizer
        .verifyHasAnyPermission(pair.first.operation, pair.second.operation)
        .test()
